...
namespace CG_N4
{
    public abstract class Utilitario
    {
        ...
        internal static void TransladarObjeto(Objeto objeto, Ponto4D pontoAtual, Ponto4D destino)
        {
            var difX = pontoAtual.X - destino.X;
            var difY = pontoAtual.Y - destino.Y;
            var difZ = pontoAtual.Z - destino.Z;

            objeto.Translacao(difX, 'x');
            objeto.Translacao(difY, 'y');
            objeto.Translacao(difZ, 'z');
        }
